...
def exams_(chat_id, group_name=None):
    try:
        if not group_name:
            if self.get_group_name(chat_id):
                group_name = self.get_group_name(chat_id)
            else:
                return 'Используй /set <группа>, чтобы установить расписание, например \'/set io61\'.'
        elif not self.is_group(group_name):
