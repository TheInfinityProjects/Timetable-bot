...
class BotCommand(object):
    ...
    def keyboard_mode(self, bot, update):
        """Handler for the /keyboard command"""
        bot.send_message(update.message.chat_id,
                         'Режим "keyboard"  активирован.',
                         reply_markup=self.markup)

    def regular_choice(self, bot, update):
        """Handler for the "Понедельник|Вторник|Среда|Четверг|Пятница|Суббота" Regex"""
        text = update.message.text
        self.lessons_on_day(bot, update, text)

    @staticmethod
    def done(bot, update):
        """Handler for the "Скрыть" Regex"""
    ...
    def lessons_next_week(self, bot, update, group_name):
        """Handler for the /nextweek command"""
        week_number = self.week()
        week_number.next()
    ...
    def lessons_tomorrow(self, bot, update, group_name):
        """Handler for the /tomorrow command"""
        week_number = self.week()
    ...
    def exams(self, bot, update, group_name):
        """Handler for the /exams command"""

        bot.send_message(update.message.chat_id,
                         text='`{}`\n{}'.format(group_name, _exams(group_name)),
                         parse_mode='Markdown')

    @is_registered
    def call_schedule(self, bot, update):
        """Send the timetable in `Markdown` format.

        Handler for the /timetable or /tt command
    ...
    def week_number(self, bot, update):
        """Handler for the /weeknumber command"""
        bot.send_message(update.message.chat_id,
